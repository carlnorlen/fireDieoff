...
--North Sierra
AND (P.ECOSUBCD = 'M261Ea' OR P.ECOSUBCD = 'M261Eb' OR P.ECOSUBCD = 'M261Ec' OR P.ECOSUBCD = 'M261Ed' OR P.ECOSUBCD = 'M261Ef' OR P.ECOSUBCD = 'M261Eg' OR P.ECOSUBCD = 'M261Eh' OR P.ECOSUBCD = 'M261Ej' OR P.ECOSUBCD = 'M261Ek' OR P.ECOSUBCD = 'M261El' OR P.ECOSUBCD = 'M261Em' OR P.ECOSUBCD = 'M261Et')
...
all$tree_type <- recode(.x=all$COMMON_NAME, 'California black oak' = 'oak', 'California juniper' = 'juniper', 'California live oak' = 'oak', 'California sycamore' = 'deciduous', 'Coulter pine' = 'pine', 'chinkapin oak' = 'oak', 'Jeffrey pine' = 'pine',
                               ...
                               "red alder" = 'deciduous', "bitter cherry" = 'deciduous', 'Rocky Mountain maple' = 'deciduous', 'unknown dead conifer' = 'other conifer', 'bishop pine' = 'pine', 'giant chinkapin, golden chinkapin' = 'deciduous', 'knobcone pine' = 'pine', 'Pacific madrone' = 'deciduous', 'Pacific yew' = 'other conifer',
                               'subalpine fir' = 'fir', 'tanoak' = 'oak', 'walnut spp.' = 'deciduous', 'Washoe pine' = 'pine')
all$tree_type <- as.factor(all$tree_type)
# test <- all %>% select(PLOT, INVYR) %>% group_by(PLOT, INVYR) %>% summarize(count = n())
live <- all %>% filter(STATUSCD == 1) %>% group_by(INVYR, PLOT, tree_type, .drop = FALSE) %>% summarize(count.live = n(), tpa.live = sum(count), basal_area.live = sum(basal_area), STDAGE = median(STDAGE))
live
#There is a slightly different result when using INVYR instead of MORTYR to calculate annual mortality
dead <- all %>% filter(STATUSCD == 2 & MORTYR %in% c("2013", "2014", "2015", "2016", "2017", "2018", "2019")) %>% group_by(PLOT, INVYR, tree_type, .drop = FALSE) %>% summarize(count.dead = n(), tpa.dead = sum(count), basal_area.dead = sum(basal_area))
...
ggsave(filename = 'Fig15_mortality_time_series_FIA_bySpecies.png', height=18, width= 10, units = 'cm', dpi=900)

#SCatter plots of Standage and die-off adn basal area
p4 <- ggplot() + geom_point(data = join %>% filter(tree_type %in% c('cedar', 'fir', 'oak', 'juniper', 'pine')) %>%   
                              filter(INVYR %in% c(2013,2014,2015,2016,2017,2018,2019) & STDAGE <= 250 & STDAGE > 0), mapping = aes(x = STDAGE, y = basal_area.all, color = tree_type)) +
      theme_bw()
p4

p5 <- ggplot() + geom_point(data = join %>% filter(tree_type %in% c('cedar', 'fir', 'oak', 'juniper', 'pine')) %>%   
                              filter(INVYR %in% c(2013,2014,2015,2016,2017,2018,2019) & STDAGE <= 250 & STDAGE > 0), mapping = aes(x = STDAGE, y = tpa.all, color = tree_type)) +
      theme_bw()
p5

p6 <- ggplot() + geom_point(data = join %>% filter(tree_type %in% c('cedar', 'fir', 'oak', 'juniper', 'pine')) %>%   
                              filter(INVYR %in% c(2013,2014,2015,2016,2017,2018,2019) & STDAGE <= 250 & STDAGE > 0), mapping = aes(x = STDAGE, y = basal_area.dead, color = tree_type)) +
...
p7 <- ggplot() + geom_point(data = join %>% filter(tree_type %in% c('cedar', 'fir', 'oak', 'juniper', 'pine')) %>%  
                              filter(INVYR %in% c(2013,2014,2015,2016,2017,2018,2019) & STDAGE > 0), mapping = aes(x = basal_area.all, y = basal_area.dead, color = tree_type)) +
  theme_bw()
p7
...
ggsave(filename = 'Fig16_mortality_standage_FIA.png', height=18, width= 18, units = 'cm', dpi=900)
...
std.q <- as.data.frame(unname(quantile((join %>% filter(INVYR %in% c(2015,2016,2017,2018,2019) & !is.na(STDAGE) & STDAGE > 0) %>% distinct(PLOT, .keep_all = TRUE))$STDAGE, prob = seq(0,1, 1/20), type = 3, na.rm = TRUE)))
# precip.q
...
join <- join %>% mutate(std.bin = case_when(
  STDAGE >= std.q %>% filter(Quintile == 0.9) %>% dplyr::select(STDAGE) %>% as.numeric() ~ '165+',
  STDAGE >= std.q %>% filter(Quintile == 0.8) %>% dplyr::select(STDAGE) %>% as.numeric() &
    STDAGE < std.q %>% filter(Quintile == 0.9) %>% dplyr::select(STDAGE) %>% as.numeric() ~ '135-164',
  STDAGE >= std.q %>% filter(Quintile == 0.7) %>% dplyr::select(STDAGE) %>% as.numeric() & 
    STDAGE < std.q %>% filter(Quintile == 0.8) %>% dplyr::select(STDAGE) %>% as.numeric() ~ '118-134',
  STDAGE >= std.q %>% filter(Quintile == 0.6) %>% dplyr::select(STDAGE) %>% as.numeric() & 
    STDAGE < std.q %>% filter(Quintile == 0.7) %>% dplyr::select(STDAGE) %>% as.numeric() ~ '108-117',
  STDAGE >= std.q %>% filter(Quintile == 0.5) %>% dplyr::select(STDAGE) %>% as.numeric() & 
    STDAGE < std.q %>% filter(Quintile == 0.6) %>% dplyr::select(STDAGE) %>% as.numeric() ~ '95-107',
  STDAGE >= std.q %>% filter(Quintile == 0.4) %>% dplyr::select(STDAGE) %>% as.numeric() & 
    STDAGE < std.q %>% filter(Quintile == 0.5) %>% dplyr::select(STDAGE) %>% as.numeric() ~ '85-94',
  STDAGE >= std.q %>% filter(Quintile == 0.3) %>% dplyr::select(STDAGE) %>% as.numeric() & 
    STDAGE < std.q %>% filter(Quintile == 0.4) %>% dplyr::select(STDAGE) %>% as.numeric() ~ '75-84',
  STDAGE >= std.q %>% filter(Quintile == 0.2) %>% dplyr::select(STDAGE) %>% as.numeric() & 
    STDAGE < std.q %>% filter(Quintile == 0.3) %>% dplyr::select(STDAGE) %>% as.numeric() ~ '61-74',
  STDAGE >= std.q %>% filter(Quintile == 0.1) %>% dplyr::select(STDAGE) %>% as.numeric() & 
    STDAGE < std.q %>% filter(Quintile == 0.2) %>% dplyr::select(STDAGE) %>% as.numeric() ~ '35-60',
  STDAGE >= std.q %>% filter(Quintile == 0.05) %>% dplyr::select(STDAGE) %>% as.numeric() &
    STDAGE < std.q %>% filter(Quintile == 0.1) %>% dplyr::select(STDAGE) %>% as.numeric() ~ '20-34',
  STDAGE < std.q %>% filter(Quintile == 0.05) %>% dplyr::select(STDAGE) %>% as.numeric() ~ '0-19'))

#Order the stand age bins
join$std.bin = with(join, factor(std.bin, levels = c('0-19', '20-34', '35-60', '61-74', '75-84', '85-94',  '95-107', '108-117','118-134', '135-164','165+')))
summary(join)
...
ggsave(filename = 'Fig17_species_mortality_standage_pointrange_FIA.png', height=18, width= 22, units = 'cm', dpi=900)
...
ggsave(filename = 'Fig18_species_mortality_standage_pointrange_FIA.png', height=18, width= 22, units = 'cm', dpi=900)
