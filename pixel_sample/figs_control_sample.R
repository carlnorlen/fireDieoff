...
#Add the data
# control.data <- read.csv(file.path(dir_in, "Stratified_sample_stand_age_2012_no_fire_history_mask_20210629_30m_v2.csv"), header = TRUE, na.strings = "NaN") #v2 is for all of Sierra and Socal
# control.data <- read.csv(file.path(fire_in, "Stratified_sample_stand_age_no_fire_history_mask_01242022_30m.csv"), header = TRUE, na.strings = "NaN")
# control.data <- read.csv(file.path(dir_in, "fraprx_ecoregion_stratified_sample_100pts_30m_ts8_20220713.csv"), header = TRUE, na.strings = "NaN")
control.data <- read.csv(file.path(dir_in, "fraprx_ecoregion_stratified_sample_4categories_0pt5pct_300m_ts16_20220722.csv"), header = TRUE, na.strings = "NaN")
# list.files(fire_in)
...
control.data$year <- as.numeric(control.data$year) + 1984 

#Convert missing TPA data to NAs
control.data[control.data$tpa_max == -9999,]$tpa_max <- NA

#Convert fire data -9999 to NAs
control.data[control.data$fire_type_2010 == -9999,]$fire_type_2010 <- NA
control.data[control.data$fire_year_2010 == -9999,]$fire_year_2010 <- NA
control.data[control.data$fire_type_2020 == -9999,]$fire_type_2020 <- NA
control.data[control.data$fire_year_2020 == -9999,]$fire_year_2020 <- NA

#Convert to trees per hectare
control.data$tpa_max <- control.data$tpa_max * 2.47105

#Make the dates into date time format for R
control.data$date <- as.Date(as.character(control.data$year), format = '%Y')
# control.data$vi.year <- format(control.data$date , '%Y')
control.data$vi.year <- control.data$year
#Use the FRAP fire perimeter year
control.data$fire.year <- control.data$fire_year_2010
control.data$stand.age <- as.numeric(control.data$year) - as.numeric(control.data$fire.year) 

#Update Cover data to 100% scale
control.data$Tree_Cover <- control.data$Tree_Cover / 100
control.data$Shrub_Cover <- control.data$Shrub_Cover / 100
control.data$Herb_Cover <- control.data$Herb_Cover / 100
control.data$Bare_Cover <- control.data$Bare_Cover / 100

#Convert the SPI48 scale back to decimal
control.data$SPI48 <- control.data$SPI48 / 100

#Try to fix soil moisture by dividing by 10
control.data$Soil_Moisture <- control.data$Soil_Moisture / 10
...
p3 <- ggplot() + 
  ...
  geom_line(data = control.data %>%
              filter(!is.na(tpa_max) & (fire.year <= 1980 | is.na(fire.year))) %>% # & vi.year >= 2003) %>%
              group_by(date) %>%
  ...
  #Dead Trees 95% CI
  geom_ribbon(data = control.data %>%
                filter(!is.na(tpa_max) & (fire.year <= 1980 | is.na(fire.year))) %>% 
                group_by(date) %>%
...
# control.data %>%
#   filter(!is.na(Tree_Cover) & fire.year <= 2010 & fire.year >= 1950 & stand.age >= 2 & !is.na(fire.year.bin)) %>% 
...
p4 <- ggplot() + 
  ...
  geom_line(data = control.data %>%
              filter(!is.na(Tree_Cover) & (fire.year <= 1980 | is.na(fire.year))) %>% 
              group_by(date) %>%
              summarize(Tree_Cover.mean = mean(Tree_Cover), Tree_Cover.n = n()),
...
f2
#Save the data
ggsave(filename = 'Fig23_dieoff_tree_cover_stand_age_time_series_control_sample.png', height=12, width= 14, units = 'cm', dpi=900)

#Create a Precip time series figure
p5 <- ggplot() + 
  geom_hline(yintercept = 0) +
  geom_line(data = control.data %>%
              filter(!is.na(ppt) & (fire.year <= 1980 | is.na(fire.year))) %>% 
...
p6 <- ggplot() + 
  geom_hline(yintercept = 0) +
  geom_line(data = control.data %>%
              filter(!is.na(AET) & (fire.year <= 1980 | is.na(fire.year))) %>% 
                       # fire.year %notin% c(1985, 1986, 1987, 1988, 1989, 1990, 1991, 1992, 1993, 1994, 1995, 1996, 1997, 1998, 1999, 2000, 2001, 2002, 2003)) %>%
              group_by(date) %>%
...
p7 <- ggplot() + 
  ...
  geom_hline(yintercept = 0) + #geom_vline(xintercept = 0, linetype = 'dashed') +
  geom_line(data = control.data %>%
              filter(!is.na(Soil_Moisture) & (fire.year <= 1980 | is.na(fire.year))) %>% 
              group_by(date) %>%
...
p8 <- ggplot() + 
  ...
  geom_hline(yintercept = 0) + #geom_vline(xintercept = 0, linetype = 'dashed') +
  geom_line(data = control.data %>%
              filter(!is.na(Water_Stress) & (fire.year <= 1980 | is.na(fire.year))) %>% 
              group_by(date) %>%
...
f3
#Save the data
ggsave(filename = 'Fig24_water_stress_stand_age_control_sample_time_series.png', height=22, width= 16, units = 'cm', dpi=900)

#Creating a fire year dTree plot
p11 <- ggplot(data = control.data %>% dplyr::filter(fire.year <= 1980 | is.na(fire.year)) %>% dplyr::group_by(system.index) %>% 
                summarize(dTree = Tree_Cover[vi.year == 2017] - Tree_Cover[vi.year == 2015], Water_Stress.4yr = Water_Stress[vi.year == 2015])) +
  ...
  theme_bw()
p11
ggsave(filename = 'Fig25_water_stress_stand_age_control_sample.png', height=16, width= 18, units = 'cm', dpi=900)

# control.data %>% summary()
p12 <- ggplot(data = control.data %>% dplyr::filter(fire.year <= 1980 | is.na(fire.year)) %>% dplyr::group_by(system.index) %>% summarize(dTree = Tree_Cover[vi.year == 2017] - Tree_Cover[vi.year == 2015], Water_Stress = Water_Stress[vi.year == 2015], SPI48 = SPI48[vi.year == 2015])) +
...
ggsave(filename = 'Fig26_SPI48_stand_age_control_sample.png', height=16, width= 18, units = 'cm', dpi=900)

control.data$PrET <- control.data$ppt - control.data$AET
p13 <- ggplot(data = control.data %>% dplyr::filter(fire.year <= 1980 | is.na(fire.year)) %>% dplyr::group_by(system.index) %>% summarize(dTree = Tree_Cover[vi.year == 2017] - Tree_Cover[vi.year == 2015], Water_Stress = Water_Stress[vi.year == 2015], SPI48 = SPI48[vi.year == 2015], PrET.4yr = sum(PrET[vi.year %in% c(2012,2013,2014,2015)]))) +
...
ggsave(filename = 'Fig27_PrET4yr_stand_age_control_sample.png', height=16, width= 18, units = 'cm', dpi=900)