...
rx.data <- read.csv(file.path(dir_in, "fire_south_sierra_FRAP_rxfire_500pt_ts8_300m_20230202.csv"), header = TRUE, na.strings = "NaN")
# fire.data$fire.year <- fire.data$perimeter_year
rx.data$treatment <- 'Disturb'
# list.files(fire_in)
# list.files(fire_in)
rx.control.data <- read.csv(file.path(dir_in, "control_south_sierra_Rx_2km_buffer_500pt_ts16_300m_20230202.csv"), header = TRUE, na.strings = "NaN")
...
p8 <- ggplot() + 
  ...
  geom_rect(data = data.frame(xmin = as.Date('2011-10-01'), xmax = as.Date('2015-09-30'), ymin = -Inf, ymax = Inf),
            fill = "red", alpha = 0.3, mapping = aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax)) +
  xlim(as.Date('2010-08-01'),as.Date('2020-01-01')) + ylim(450, 625) + 
...
p16 <- ggplot(data = rx.pixel.data %>% filter(Tree_Cover > 0 & fire.year <= 2010 & fire.year >= 1921 & fire_type_2010 == 2 & (fire_year_2019 <= 2010 | is.na(fire_year_2019))) %>% # &
  ...
  scale_fill_gradient2(limits = c(0,250), breaks = c(0,50,100,150,200), midpoint = 125, low = "cornflowerblue", mid = "yellow", high = "red", na.value = 'transparent') + #
...
p16
ggsave(filename = 'Fig6b_water_stress_dTree_300m.png', height=16, width= 24, units = 'cm', dpi=900)
...
#Save the data
ggsave(filename = 'Fig8b_rx_stand_age_veg_cover.png', height=18, width= 20, units = 'cm', dpi=900)
...
#Save the data
ggsave(filename = 'Fig9b_rx_stand_age_tree_cover.png', height=18, width= 20, units = 'cm', dpi=900)
...
#Save the data
ggsave(filename = 'Fig10b_rx_stand_age_AET.png', height=18, width= 20, units = 'cm', dpi=900)
...
#Save the data
ggsave(filename = 'Fig11b_rx_stand_age_shrub.png', height=18, width= 20, units = 'cm', dpi=900)
