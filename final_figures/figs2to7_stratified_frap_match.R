...
fire.data <- read.csv(file.path(dir_in, "fire_south_sierra_FRAP_wildfire_400pt_ts8_300m_20230112.csv"), header = TRUE, na.strings = "NaN")
# fire.data <- read.csv(file.path(dir_in, "fire_south_sierra_FRAP_wildfire_10pct_ts8_300m_20221107.csv"), header = TRUE, na.strings = "NaN")
# fire.data$fire.year <- fire.data$perimeter_year
fire.data$treatment <- 'Disturb'
# list.files(fire_in)
# list.files(fire_in)
control.data <- read.csv(file.path(dir_in, "control_south_sierra_no_FRAP_undist_400pt_ts16_300m_20230117.csv"), header = TRUE, na.strings = "NaN")
# control.data <- read.csv(file.path(dir_in, "control_south_sierra_no_FRAP_undist_10pct_ts8_300m_20221107.csv"), header = TRUE, na.strings = "NaN")

# control_south_sierra_no_FRAP_undist_10pct_ts8_300m_20221107
...
fire.data$buffer_type_2010 <- -9999
fire.data$buffer_type_2019 <- -9999
fire.data$buffer_year_2010 <- -9999
fire.data$buffer_year_2019 <- -9999
...
pixel.data <- rbind(fire.data, control.data)
...
pixel.data$fire.year <- pixel.data$fire_year_2019
...
# pixel.data$Water_Stress <- pixel.data$Water_Stress
# pixel.data$ppt <- pixel.data$ppt
# pixel.data$AET <- pixel.data$AET
# pixel.data$GPP <- pixel.data$GPP
# pixel.data$elevation <- pixel.data$elevation
...
pixel.data <- pixel.data %>% mutate(fire.year.bin = case_when(
  ...
  # bin == 2 ~ '1909-1910',
  # bin >= 1911 & bin <= 1920 ~ '95-104', #Calculated relative to 2015
  is.na(fire.year) | fire.year < 1980 ~ 'No Fire',
...
pixel.data$fire.year.bin = with(pixel.data, factor(fire.year.bin, levels = c('2019-2020', '2011-2018', '1980-2010',  'No Fire')))#c('0-4','5-30','31-55','56-80', '1951-1980', '1921-1950', '1900-1919',
...
p1 <- ggplot() +
  ...
  geom_bin2d(data = pixel.data %>% 
               filter((Tree_Cover >= 0 & treatment == 'Disturb' & fire.year <= 2010 & fire.year >= 1921 & elevation <= 3000) | (Tree_Cover >= 0 & is.na(fire.year) & elevation <= 3000)) %>% # &
...
p2<- ggplot() +
  #Data Summary
  geom_bin2d(data = pixel.data %>% 
               filter((!is.na(Tree_Cover) & treatment == 'Disturb' & fire.year <= 2010 & fire.year >= 1921 & elevation <= 3000) | (!is.na(Tree_Cover) & is.na(fire.year) & elevation <= 3000)) %>% # &
               filter(lf_evt_2001 %in% c(2031, 2173, 2027, 2019, 2032, 2033, 2172, 2053)) %>% #Filter by the tree types
               # elevation <= elev.upper & clm_precip_sum_mean >= ppt.lower & #elevation >= elev.lower &
               # if_else(treatment == 'Wildfire', fire.year == fire_year_2019_mode, is.na(fire_year_2019_mode))) %>%
               dplyr::group_by(system.index, fire.year.bin) %>% 
...
p3 <- ggplot() +
  geom_bin2d(data = pixel.data %>% 
               filter((treatment == 'Disturb' & fire.year <= 2010 & fire.year >= 1921 & !is.na(tpa_max) & elevation <= 3000) | (is.na(fire.year) & !is.na(tpa_max) & elevation <= 3000)) %>% # &
               filter(lf_evt_2001 %in% c(2031, 2173, 2027, 2019, 2032, 2033, 2172, 2053)) %>% #Filter by the tree types
               # elevation <= elev.upper & clm_precip_sum_mean >= ppt.lower & #elevation >= elev.lower &
               # if_else(treatment == 'Wildfire', fire.year == fire_year_2019_mode, is.na(fire_year_2019_mode))) %>%
               dplyr::group_by(system.index, fire.year.bin) %>%
...
p5 <- ggplot() + 
  ...
  geom_line(data = pixel.data %>%
              filter((!is.na(tpa_max) & treatment == 'Disturb' & fire.year <= 2010 & fire.year >= 1921 & stand.age > 2 & vi.year >= 2010 & elevation <= 3000) | 
                       (!is.na(tpa_max) & treatment == 'Control' & is.na(fire.year) & vi.year >= 2010 & elevation <= 3000)) %>% # & stratlayer %in% strat.list  & stratlayer %in% strat.list
...
p6 <- ggplot() + 
  ...
  geom_line(data = pixel.data %>%
              filter((treatment == 'Disturb' & fire.year <= 2010 & fire.year >= 1921 & stand.age > 2 & !is.na(Tree_Cover) & Tree_Cover > 0 & vi.year >= 2010 & elevation <= 3000) | 
                       (treatment == 'Control' & is.na(fire.year)& !is.na(Tree_Cover) & Tree_Cover > 0 & vi.year >= 2010 & elevation <= 3000)) %>% # & stratlayer %in% strat.list & stratlayer %in% strat.list
              filter(lf_evt_2001 %in% c(2031, 2173, 2027, 2019, 2032, 2033, 2172, 2053)) %>%
              # elevation <= elev.upper & clm_precip_sum_mean >= ppt.lower & 
              # if_else(treatment == 'Wildfire', fire.year == fire_year_2019_mode, is.na(fire_year_2019_mode))) %>% 
              group_by(date, fire.year.bin) %>%
  ...
  geom_ribbon(data = pixel.data %>%
                filter((treatment == 'Disturb' & fire.year <= 2010 & fire.year >= 1921 & stand.age > 2 & !is.na(Tree_Cover) & Tree_Cover > 0 & vi.year >= 2010 & elevation <= 3000) | 
                         (treatment == 'Control' & is.na(fire.year)& !is.na(Tree_Cover) & Tree_Cover > 0 & vi.year >= 2010 & elevation <= 3000)) %>% 
                filter(lf_evt_2001 %in% c(2031, 2173, 2027, 2019, 2032, 2033, 2172, 2053)) %>%
                # elevation <= elev.upper & clm_precip_sum_mean >= ppt.lower & 
                # if_else(treatment == 'Wildfire', fire.year == fire_year_2019_mode, is.na(fire_year_2019_mode))) %>% 
                group_by(date, fire.year.bin) %>%
...
f2
#Save the data
ggsave(filename = 'Fig3a_frap_fire_dieoff_tree_cover_stand_age_time_series_.png', height=12, width= 10, units = 'cm', dpi=900)

#Figure 4: Precip, ET, Soil moisture, Water Stress time series figure
p7 <- ggplot() + 
  geom_hline(yintercept = 0) +
  geom_line(data = pixel.data %>%
              filter((treatment == 'Disturb' & fire.year <= 2010 & fire.year >= 1921 & stand.age > 2 & vi.year >= 2010 & elevation <= 3000) | 
                       (treatment == 'Control' & is.na(fire.year) & vi.year >= 2010 & elevation <= 3000)) %>% # &
              filter(lf_evt_2001 %in% c(2031, 2173, 2027, 2019, 2032, 2033, 2172, 2053)) %>%         
              # elevation <= elev.upper & clm_precip_sum_mean >= ppt.lower & 
              # if_else(treatment == 'Wildfire', fire.year == fire_year_2019_mode, is.na(fire_year_2019_mode))) %>% 
              group_by(date, fire.year.bin) %>%
              summarize(ppt.mean = mean(ppt), ppt.n = n(), count = n()) %>%  
              filter(if_else(fire.year.bin == '1980-2010', count >= 2500, count >= 0)),
...
f3
#Save the data
ggsave(filename = 'Fig4a_water_stress_stand_age_frap_perimeter_10pt_sample_300m_time_series.png', height=22, width= 10, units = 'cm', dpi=900)
...
p15 <- ggplot() +
  ...
  theme(axis.text.y = element_text(size = 8), axis.title.y = element_text(size = 10),
        ...
        legend.key = element_rect(fill = NA), axis.text.x = element_text(size = 8),
        legend.title = element_text(size = 8), legend.text = element_text(size = 6)) +
  xlab('Fire Year') + ylab('Water Stress (mm)') 
p15
...
#Tree Cover Die-off
p17 <- ggplot(data = pixel.data %>% filter((fire.year.bin == '1980-2010' & fire.year <= 2010 & fire.year >= 1921 & stand.age > 2 & Tree_Cover > 0)) %>% # &
               filter(lf_evt_2001 %in% c(2031, 2173, 2027, 2019, 2032, 2033, 2172, 2053)) %>%
                filter(elevation <= 3000) %>%
               dplyr::group_by(system.index) %>% 
               summarize(dTree = (mean(Tree_Cover[vi.year %in% c(2017, 2018)]) - mean(Tree_Cover[vi.year %in% c(2013,2014)])), 
...
ggsave(filename = 'Fig7a_stand_age_dTree_300m.png', height=16, width= 18, units = 'cm', dpi=900)
...
p1a <- ggplot() + 
  ...
  geom_line(data = pixel.data %>%
              filter((treatment == 'Disturb' & vi.year <= 2014 & fire.year <= 2010 & fire.year >= 1921)) %>%
...
#   #Create a shrub cover line
#   geom_line(data = pixel.data %>%
#               filter(vi.year <= 2014 & fire.year <= 2010 & fire.year >= 1921) %>%
#               group_by(stand.age, treatment) %>%
#               dplyr::select(FIRE_NUM) %>% 
#               unique() %>%
...
f1a
#Save the data
ggsave(filename = 'Fig77_data_check_10pt_frap_perimeter_chronosequence.png', height=22, width= 16, units = 'cm', dpi=900)
...
p35 <- ggplot() + 
  ...
  geom_point(data = pixel.data %>%
               ...
               filter(lf_evt_2001 %in% c(2031, 2173, 2027, 2019, 2032, 2033, 2172, 2053)) %>% #Select specific forest types
...
#Save the data
ggsave(filename = 'Fig78_fire year_1980_tree_cover_frap_perimeter.png', height=18, width= 20, units = 'cm', dpi=900)

#Fire out which strat layers have data for at least on fire year group and No Fires
# group.test <- pixel.data %>%
...
#   group_by(stratlayer) %>%
#   filter(any(count >= 175 & fire.year.bin == 'No Fire') & any(count >= 175 & fire.year.bin =='1981-2010') & 
#          any(count >= 175 & fire.year.bin =='1951-1980') & any(count >= 175 & fire.year.bin =='1921-1950'))
# group.test
# #Select the stratlayer that have at least No Fire and One Fire Year Group
# strat.list <- group.test$stratlayer %>% unique()
...
p36 <- ggplot() + 
  ...
  geom_hline(yintercept = 0) + #geom_vline(xintercept = 0, linetype = 'dashed') +
  geom_point(data = pixel.data %>%
               dplyr::filter(((!is.na(Tree_Cover) & fire.year <= 2010 & fire.year >= 1980 ) | (!is.na(Tree_Cover) & is.na(fire.year))) #& & stratlayer %in% strat.list & stratlayer %in% strat.list)
                             # if_else(treatment == 'Wildfire', fire.year == fire_year_2019_mode, is.na(fire_year_2019_mode))
               ) %>%
               filter(lf_evt_2001 %in% c(2031, 2173, 2027, 2019, 2032, 2033, 2172, 2053)) %>%
...
#Save the data
ggsave(filename = 'Fig79_fire year_tree_cover_frap_perimeter.png', height=36, width= 36, units = 'cm', dpi=900)


#Elevation limits
# elev.lower <- (pixel.data %>% filter(fire.year >= 1921 & fire.year <= 1950))$elevation %>% quantile(0.0)
# elev.lower
# elev.upper <- (pixel.data %>% filter(fire.year >= 1921 & fire.year <= 1950))$elevation %>% quantile(0.95)
# elev.upper
# ppt.upper <- (pixel.data %>% filter(fire.year >= 1921 & fire.year <= 1950))$clm_precip_sum_mean %>% quantile(1)
# ppt.upper
# ppt.lower <- (pixel.data %>% filter(fire.year >= 1921 & fire.year <= 1950))$clm_precip_sum_mean %>% quantile(0.05)
# ppt.lower
# temp.lower <- (pixel.data %>% filter(fire.year >= 1921 & fire.year <= 1950))$clm_temp_mean_mean %>% quantile(0.0)
# temp.lower
...
p2 <- ggplot() + 
  ...
  #Create a shrub cover line
  geom_line(data = pixel.data %>%
              filter(stand.age >= -10 & stand.age <= 85 & !is.na(Shrub_Cover) & vi.year <= 2014 & fire.year >= 1980 & fire.year <= 2010 ) %>% #& elevation >= elev.lower & clm_temp_mean_mean >= temp.lower & clm_precip_sum_mean <= ppt.upper & stratlayer %in% strat.list
              filter(lf_evt_2001 %in% c(2031, 2173, 2027, 2019, 2032, 2033, 2172, 2053)) %>%         
...
#Save the data
ggsave(filename = 'Fig80_veg_cover_stand_age_10pt_300m_frap_1980_perimeter.png', height=18, width= 20, units = 'cm', dpi=900)

#Checking for numbers of samples.
p9 <- ggplot() +
  geom_hline(yintercept = 0) +
  geom_line(data = pixel.data %>%
              filter(!is.na(Tree_Cover) & fire.year <= 2010 & fire.year >= 1921 & stand.age > 2 ) %>% # && stratlayer %in% strat.list
              filter(lf_evt_2001 %in% c(2031, 2173, 2027, 2019, 2032, 2033, 2172, 2053)) %>%
                       # elevation <= elev.upper & clm_precip_sum_mean >= ppt.lower &
                       # if_else(treatment == 'Wildfire', fire.year == fire_year_2019_mode, is.na(fire_year_2019_mode))) %>%
...
f4
# 
ggsave(filename = 'Fig85_data_check_time_series_perimeter_10pt_300m.png', height=16, width= 16, units = 'cm', dpi=900)

# summary(pixel.data)
#Figure of Dead Trees per acre separated by fire years with time series
# #Save the data
# ggsave(filename = 'Fig81_dieoff_tree_cover_stand_age_time_series_frap_perimeter_10pt_sample_300m.png', height=12, width= 14, units = 'cm', dpi=900)
...
#               group_by(date, fire.year.bin) %>%
#               summarize(ppt.mean = mean(ppt), ppt.n = n(), count = n()) %>%  
#               filter(if_else(fire.year.bin == '1980-2010', count >= 3000, count >= 0)),
...
#   fire.year >= 1921 & fire.year <= 1965 ~ '1921-1965',
...
# #fire.year >= 2019 ~ '2019-2020'))#'0-4'))
# 
# pixel.data$fire.year.grp = with(pixel.data, factor(fire.year.grp, levels = c('2011-2020', '1966-2010', '1921-1965', '1900-1920', 'No Fire')))#c('0-4','5-30','31-55','56-80',
...
ggsave(filename = 'Fig82_stand_age_bins_dieoff_treecover_water_stress_10pt_300m.png', height=24, width = 18, units = 'cm', dpi=900)
# summary(pixel.data)
...
#Figure of Dead Trees per acre separated by fire years with time series
#Create a manual color scale
cols.1 <- c("1981-2010"="black","1951-1980"="gray", "1921-1950" = "white")
lines.1 <- c("1981-2010"="solid","1951-1980"="dashed", "1921-1950" = "dotted")
#ADS Difference
p24 <- ggplot() + 
  geom_hline(yintercept = 0) +
  geom_line(data = pixel.data %>%
              dplyr::filter((!is.na(tpa_max) & fire.year <= 2010 & fire.year >= 1921 & stand.age > 2 & stratlayer %in% strat.list) | ( !is.na(tpa_max) &is.na(fire.year) & stratlayer %in% strat.list)) %>% # &
                              # if_else(treatment == 'Wildfire', fire.year == fire_year_2019_mode, is.na(fire_year_2019_mode))) %>%    
              group_by(date, fire.year.bin) %>%
              summarize(tpa_max.mean = mean(tpa_max), tpa_max.n = n()) %>% 
              # ungroup() %>%
              group_by(date) %>%
              summarize(tpa_max.2010 = tpa_max.mean[fire.year.bin == '1981-2010'] - tpa_max.mean[fire.year.bin == 'No Fire']), #%>%
              # group_by(date, fire.year.bin) %>%
              # summarize(tpa_max.diff.mean = mean(tpa_max.diff)),
            # filter(if_else(fire.year.bin == '1985-2010', tpa_max.n >= 6000, tpa_max.n >= 0)), 
            mapping = aes(x = date, y = tpa_max.2010, color = '1981-2010', linetype = '1981-2010'), 
            size = 1
  ) +
  #Dead Trees 95% CI
  # geom_ribbon(data = pixel.data %>%
  ...
  #                           x = date, fill = fire.year.bin), alpha = 0.3) +
  #Do the subtraction versus 1951-1980  
  geom_line(data = pixel.data %>%
            dplyr::filter((!is.na(tpa_max) & fire.year <= 2010 & fire.year >= 1921 & stand.age > 2 & stratlayer %in% strat.list) | ( !is.na(tpa_max) &is.na(fire.year) & stratlayer %in% strat.list)) %>% # &
            # if_else(treatment == 'Wildfire', fire.year == fire_year_2019_mode, is.na(fire_year_2019_mode))) %>%    
            group_by(date, fire.year.bin) %>%
            ...
            # ungroup() %>%
            group_by(date) %>%
            summarize(tpa_max.1980 = tpa_max.mean[fire.year.bin == '1951-1980'] - tpa_max.mean[fire.year.bin == 'No Fire']), #%>%
          # group_by(date, fire.year.bin) %>%
          # summarize(tpa_max.diff.mean = mean(tpa_max.diff)),
          # filter(if_else(fire.year.bin == '1985-2010', tpa_max.n >= 6000, tpa_max.n >= 0)), 
          mapping = aes(x = date, y = tpa_max.1980, color = '1951-1980', linetype = '1951-1980'), 
          size = 1
  ) +
  #Do subtraction versus 1921-1950
  geom_line(data = pixel.data %>%
              dplyr::filter((!is.na(tpa_max) & fire.year <= 2010 & fire.year >= 1921 & stand.age > 2 & stratlayer %in% strat.list) | ( !is.na(tpa_max) &is.na(fire.year) & stratlayer %in% strat.list)) %>% # &
              # if_else(treatment == 'Wildfire', fire.year == fire_year_2019_mode, is.na(fire_year_2019_mode))) %>%    
              group_by(date, fire.year.bin) %>%
              ...
              # ungroup() %>%
              group_by(date) %>%
              summarize(tpa_max.1950 = tpa_max.mean[fire.year.bin == '1921-1950'] - tpa_max.mean[fire.year.bin == 'No Fire']), #%>%
            # group_by(date, fire.year.bin) %>%
            # summarize(tpa_max.diff.mean = mean(tpa_max.diff)),
            # filter(if_else(fire.year.bin == '1985-2010', tpa_max.n >= 6000, tpa_max.n >= 0)), 
            mapping = aes(x = date, y = tpa_max.1950, color = '1921-1950', linetype = '1921-1950'), 
            size = 1
  ) +
  #Do the Formating
  ...
  geom_rect(data = data.frame(xmin = as.Date('2011-10-01'), xmax = as.Date('2015-09-30'), ymin = -Inf, ymax = Inf),
            fill = "red", alpha = 0.3, mapping = aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax)) +
  xlim(as.Date('1985-08-01'),as.Date('2020-01-01')) + #facet_grid(. ~ fire.year.bin) +
  ylab(expression(atop('ADS Die-off (Wildfire - Control)', '(trees ha'^-1*')'))) + xlab('Year') #+ facet_wrap(. ~ fire_type_last, labeller = as_labeller(c('1' = 'Wild', '2' = 'Prescribed')))
p24

# Difference between wilfire and control

p25 <- ggplot() + 
  # geom_line(mapping = aes(group = .geo), color = 'dark gray', size = 0.2, alpha = 0.2) +
  geom_hline(yintercept = 0) + #geom_vline(xintercept = 0, linetype = 'dashed') +
  #1981-2010 Tree Cover Difference
  geom_line(data = pixel.data %>%
              dplyr::filter((!is.na(Tree_Cover) & fire.year <= 2010 & fire.year >= 1921 & stand.age > 2 & stratlayer %in% strat.list) | (!is.na(Tree_Cover) & is.na(fire.year) & stratlayer %in% strat.list)) %>% # &
              # if_else(treatment == 'Wildfire', fire.year == fire_year_2019_mode, is.na(fire_year_2019_mode))) %>%  
              group_by(date, fire.year.bin) %>%
              summarize(Tree_Cover.mean = mean(Tree_Cover), Tree_Cover.n = n()) %>%  
              filter(if_else(fire.year.bin == '1981-2010', Tree_Cover.n >= 4500, Tree_Cover.n >= 0)) %>%
              group_by(date) %>%
              summarize(Tree_Cover.2010 = Tree_Cover.mean[fire.year.bin == '1981-2010'] - Tree_Cover.mean[fire.year.bin == 'No Fire']), #%>%
              # group_by(date, fire.year.bin) %>%
              # summarize(Tree_Cover.diff.mean = mean(Tree_Cover.diff)),
            mapping = aes(x = date, y = Tree_Cover.2010, color = '1981-2010', linetype = '1981-2010'), 
            size = 1) + 
  #1951-1980 tree cover difference
  geom_line(data = pixel.data %>%
              dplyr::filter((!is.na(Tree_Cover) & fire.year <= 2010 & fire.year >= 1921 & stand.age > 2 & stratlayer %in% strat.list) | (!is.na(Tree_Cover) & is.na(fire.year) & stratlayer %in% strat.list)) %>% # &
              # if_else(fire.year.bin == 'Wildfire', fire.year == fire_year_2019_mode, is.na(fire_year_2019_mode))) %>%  
              group_by(date, fire.year.bin) %>%
              summarize(Tree_Cover.mean = mean(Tree_Cover), Tree_Cover.n = n()) %>%  
              # filter(if_else(fire.year.bin == '1981-2010', Tree_Cover.n >= 6000, Tree_Cover.n >= 0)) %>%
              group_by(date) %>%
              summarize(Tree_Cover.1980 = Tree_Cover.mean[fire.year.bin == '1951-1980'] - Tree_Cover.mean[fire.year.bin == 'No Fire']), #%>%
            # group_by(date, fire.year.bin) %>%
            # summarize(Tree_Cover.diff.mean = mean(Tree_Cover.diff)),
            mapping = aes(x = date, y = Tree_Cover.1980, color = '1951-1980', linetype = '1951-1980'), 
            size = 1) + 
  #1921-1950 tree cover difference
  geom_line(data = pixel.data %>%
              dplyr::filter((!is.na(Tree_Cover) & fire.year <= 2010 & fire.year >= 1921 & stand.age > 2 & stratlayer %in% strat.list) | (!is.na(Tree_Cover) & is.na(fire.year) & stratlayer %in% strat.list)) %>% # &
              # if_else(fire.year.bin == 'Wildfire', fire.year == fire_year_2019_mode, is.na(fire_year_2019_mode))) %>%  
              group_by(date, fire.year.bin) %>%
              ...
              # filter(if_else(fire.year.bin == '1981-2010', Tree_Cover.n >= 6000, Tree_Cover.n >= 0)) %>%
              group_by(date) %>%
              summarize(Tree_Cover.1950 = Tree_Cover.mean[fire.year.bin == '1921-1950'] - Tree_Cover.mean[fire.year.bin == 'No Fire']), #%>%
            # group_by(date, fire.year.bin) %>%
            # summarize(Tree_Cover.diff.mean = mean(Tree_Cover.diff)),
            mapping = aes(x = date, y = Tree_Cover.1950, color = '1921-1950', linetype = '1921-1950'), 
            size = 1) + 
  #Tree Cover 95% CI
  # geom_ribbon(data = pixel.data %>%
...
f7
#Save the data
ggsave(filename = 'Fig86_dieoff_tree_cover_stand_age_time_series_frap_perimeter_10pt_sample_300m.png', height=12, width= 14, units = 'cm', dpi=900)
...
p32 <- ggplot() +
  #Data Summary
  geom_bin2d(data = pixel.data %>% 
               filter((Tree_Cover >= 0 & treatment == 'Disturb' & fire.year <= 2010 & fire.year >= 1921) | (Tree_Cover >= 0 & is.na(fire.year))) %>% # &
               filter(lf_evt_2001 %in% c(2031, 2173, 2027, 2019, 2032, 2033, 2172, 2053)) %>%
               # elevation <= elev.upper & clm_precip_sum_mean >= ppt.lower & #elevation >= elev.lower &
               # if_else(treatment == 'Wildfire', fire.year == fire_year_2019_mode, is.na(fire_year_2019_mode))) %>%
               dplyr::group_by(system.index, fire.year.bin) %>% 
               summarize(dTree = (mean(Tree_Cover[vi.year %in% c(2018, 2019)]) - mean(Tree_Cover[vi.year %in% c(2014,2015)])), RdTree = (mean(Tree_Cover[vi.year %in% c(2018, 2019)]) - mean(Tree_Cover[vi.year %in% c(2014,2015)])) / mean(Tree_Cover[vi.year %in% c(2014, 2015)]), 
                         Water_Stress = Water_Stress[vi.year == 2015], Tree_Cover = (mean(Tree_Cover[vi.year %in% c(2013, 2014)])), clm_precip_sum = clm_precip_sum[vi.year == 2015],
                         clm_temp_mean = clm_temp_mean[vi.year == 2015]),
             mapping = aes(x = clm_temp_mean, y = clm_precip_sum, fill = Tree_Cover, group = Tree_Cover), binwidth = c(1,100)) + 
  # geom_errorbar(data = pixel.data %>% 
  #                 filter((treatment == 'Disturb' & fire.year <= 2010 & fire.year >= 1921 & stand.age > 2 & stratlayer %in% strat.list) | (is.na(fire.year) & stratlayer %in% strat.list)) %>% # & 
  #                 # elevation <= elev.upper & clm_precip_sum_mean >= ppt.lower & #elevation >= elev.lower &
  #                 # if_else(treatment == 'Wildfire', fire.year == fire_year_2019_mode, is.na(fire_year_2019_mode))) %>%
...
p33 <- ggplot() +
  ...
  geom_bin2d(data = pixel.data %>% 
               ...
               summarize(dTree = (mean(Tree_Cover[vi.year %in% c(2017, 2018)]) - mean(Tree_Cover[vi.year %in% c(2013,2014)])), RdTree = (mean(Tree_Cover[vi.year %in% c(2017, 2018)]) - mean(Tree_Cover[vi.year %in% c(2013,2014)])) / mean(Tree_Cover[vi.year %in% c(2013, 2014)]), 
                         ...
                         clm_temp_mean = clm_temp_mean[vi.year == 2015]),
             mapping = aes(x = clm_temp_mean, y = clm_precip_sum,  fill = dTree, group = dTree), binwidth = c(1, 100)) + 
  # geom_errorbar(data = pixel.data %>% 
  #                 filter((treatment == 'Disturb' & fire.year <= 2010 & fire.year >= 1921 & stand.age > 2 & stratlayer %in% strat.list) | (is.na(fire.year) & stratlayer %in% strat.list)) %>% # & 
  #                 # elevation <= elev.upper & clm_precip_sum_mean >= ppt.lower & #elevation >= elev.lower &
  #                 # if_else(treatment == 'Wildfire', fire.year == fire_year_2019_mode, is.na(fire_year_2019_mode))) %>%
  ...
  theme_bw() +
  # scale_color_brewer(type = 'div', palette = 'Set1', name = 'Treatment') +
  scale_fill_gradient2(name = "Die-off \n(% Tree Cover)", low = "firebrick1", mid = "lightgoldenrodyellow", high = "dodgerblue", midpoint = 0, limits = c(-10, 5), na.value = 'transparent') +  # limits = c(-15, 10), 
  theme(axis.text.y = element_text(size = 8), legend.position = "right", axis.title.y = element_text(size = 10),
        axis.title.x = element_blank(), legend.background = element_rect(colour = NA, fill = NA),
        legend.key = element_rect(fill = NA), axis.text.x = element_blank(),
        legend.title = element_text(size = 8), legend.text = element_text(size = 6)) + facet_grid(.~ fire.year.bin) +
...
p34 <- ggplot() +
  ...
  geom_bin2d(data = pixel.data %>% 
               ...
               summarize(dTree = (mean(Tree_Cover[vi.year %in% c(2018, 2019)]) - mean(Tree_Cover[vi.year %in% c(2014,2015)])), RdTree = (mean(Tree_Cover[vi.year %in% c(2018, 2019)]) - mean(Tree_Cover[vi.year %in% c(2014,2015)])) / mean(Tree_Cover[vi.year %in% c(2014, 2015)]), 
                         ...
                         latitude = latitude[vi.year == 2015], count = sum(elevation[vi.year == 2015]), clm_precip_sum = clm_precip_sum[vi.year == 2015],
                         clm_temp_mean = clm_temp_mean[vi.year == 2015]),
             mapping = aes(x = clm_temp_mean, y = clm_precip_sum), binwidth = c(1, 100)) + 
  # geom_errorbar(data = pixel.data %>% 
  #                 filter((treatment == 'Disturb' & fire.year <= 2010 & fire.year >= 1921 & stand.age > 2 & stratlayer %in% strat.list) | (is.na(fire.year) & stratlayer %in% strat.list)) %>% # & 
  #                 # elevation <= elev.upper & clm_precip_sum_mean >= ppt.lower & #elevation >= elev.lower &
  #                 # if_else(treatment == 'Wildfire', fire.year == fire_year_2019_mode, is.na(fire_year_2019_mode))) %>%
  ...
  theme(axis.text.y = element_text(size = 8), legend.position = "right", axis.title.y = element_text(size = 10),
        ...
        legend.key = element_rect(fill = NA), axis.text.x = element_text(size = 8),
        legend.title = element_text(size = 8), legend.text = element_text(size = 6)) + facet_grid(.~ fire.year.bin) +
  xlab('Average Temp (1980-2010)') + ylab('Annual Precip (1980-2010)')
p34

f9 <- ggarrange(p32, p33, p34, ncol = 1, nrow = 3, common.legend = FALSE, heights = c(0.9, 0.9, 1), align = "v", labels = c('a)', 'b)', 'c)'))
f9
#Save the data
ggsave(filename = 'Fig106_dieoff_tree_cover_fireyear_1980_climate_distribution.png', height=18, width= 24, units = 'cm', dpi=900)

#Add climate bins
pixel.test <- pixel.data %>%
  ...
  dplyr::mutate(clm_precip.bin = cut(clm_precip_sum, breaks = seq(200, 2000, by = 200)),
                clm_temp.bin = cut(clm_temp_mean, breaks = seq(0, 18, by = 2)),
  ) 

# pixel.test %>% summary()
# pixel.test %>%
#   dplyr::filter(((!is.na(Tree_Cover) & fire.year <= 2010 & fire.year >= 1921) | (!is.na(Tree_Cover) & is.na(fire.year))) #&
#                 # if_else(treatment == 'Wildfire', fire.year == fire_year_2019_mode, is.na(fire_year_2019_mode))
#   ) %>%
#   group_by(date, clm_precip.bin, clm_temp.bin, fire.year.bin) %>%
#   summarize(Tree_Cover.mean = mean(Tree_Cover), count = n())

#Tree Cover climate space test
p35 <- ggplot() + 
  # geom_line(mapping = aes(group = .geo), color = 'dark gray', size = 0.2, alpha = 0.2) +
  geom_hline(yintercept = 0) + #geom_vline(xintercept = 0, linetype = 'dashed') +
  geom_point(data = pixel.test %>%
               dplyr::filter(((!is.na(Tree_Cover) & fire.year <= 2010 & fire.year >= 1921) | (!is.na(Tree_Cover) & is.na(fire.year))) #&
                             # if_else(treatment == 'Wildfire', fire.year == fire_year_2019_mode, is.na(fire_year_2019_mode))
               ) %>%
               filter(lf_evt_2001 %in% c(2031, 2173, 2027, 2019, 2032, 2033, 2172, 2053)) %>%
               group_by(date, clm_precip.bin, clm_temp.bin, fire.year.bin) %>%
               summarize(Tree_Cover.mean = mean(Tree_Cover), count = n()) %>%
               filter(any(count >= 20 & fire.year.bin == 'No Fire') & any(count >= 20 & (fire.year.bin =='1980-2010'))),
             # filter(if_else(fire.year.bin == '1981-2010', Tree_Cover.n >= 600, Tree_Cover.n >= 0)) %>%
             # group_by(date, fire.year.bin) %>%
             # summarize(Tree_Cover.diff = Tree_Cover.mean[treatment == 'Buffer'] - Tree_Cover.mean[treatment == 'Wildfire']), #%>%
             # group_by(date, fire.year.bin) %>%
  ...
  theme(axis.text.y = element_text(size = 8), axis.title.y = element_text(size = 10),
        ...
        legend.title = element_text(size = 8), legend.text = element_text(size = 6)) +
  geom_rect(data = data.frame(xmin = as.Date('2011-10-01'), xmax = as.Date('2015-09-30'), ymin = -Inf, ymax = Inf),
            fill = "red", alpha = 0.3, mapping = aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax)) + facet_wrap(clm_precip.bin ~ clm_temp.bin) +
  xlim(as.Date('1985-08-01'),as.Date('2020-01-01')) + #facet_grid(. ~ fire.year.bin) + #ylim(25, 55) +
  ylab(expression('Tree (%)')) + xlab('Year') #+ facet_wrap(. ~ fire_type_last, labeller = as_labeller(c('1' = 'Wild', '2' = 'Prescribed')))
p35

ggsave(filename = 'Fig107_fire_year_1980_time_series_climate_distribution.png', height=36, width= 36, units = 'cm', dpi=900)
