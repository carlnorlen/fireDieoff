...
pixel.data <- read.csv(file.path(dir_in, "fraprx_ecoregion_simple_sample_by_wildfire_50pt_30m_20220726.csv"), header = TRUE, na.strings = "NaN")
# list.files(fire_in)
summary(pixel.data)
#Get a  of the data
# summary(pixel.data)
...
p2 <- ggplot() + 
  ...
  #Create a shrub cover line
  geom_line(data = pixel.data %>%
              filter(stand.age >= -10 & stand.age <= 70 & !is.na(Shrub_Cover) & vi.year <= 2010 & fire.year <= 2010 & !is.na(fire.year)) %>%
              group_by(stand.age) %>%
  ...
  geom_line(data = pixel.data %>%
              filter(stand.age >= -10 & stand.age <= 70 & !is.na(Tree_Cover) & vi.year <= 2010 & fire.year <= 2010 & !is.na(fire.year)) %>%
              group_by(stand.age) %>%
  ...
  geom_line(data = pixel.data %>%
              filter(stand.age >= -10 & stand.age <= 70 & !is.na(Herb_Cover) & vi.year <= 2010 & fire.year <= 2010 & !is.na(fire.year)) %>%
              group_by(stand.age) %>%
  ...
  geom_line(data = pixel.data %>%
              filter(stand.age >= -10 & stand.age <= 70 & !is.na(Bare_Cover) & vi.year <= 2010 & fire.year <= 2010 & !is.na(fire.year)) %>% 
              group_by(stand.age) %>%
...
#Save the data
ggsave(filename = 'Fig2_veg_cover_stand_age_50pt_frap_perimeter.png', height=12.5, width= 20, units = 'cm', dpi=900)
...
p3 <- ggplot() + 
  geom_hline(yintercept = 0) +
  geom_line(data = pixel.data %>%
              filter(!is.na(tpa_max) & fire.year <= 2010 & stand.age >= 2 & !is.na(stand.age.bin)) %>% # & vi.year >= 2003) %>%
  ...
  #Dead Trees 95% CI
  geom_ribbon(data = pixel.data %>%
                filter(!is.na(tpa_max) & fire.year <= 2010 & stand.age >= 2 & !is.na(stand.age.bin)) %>% 
                group_by(date, stand.age.bin) %>%
...
p4 <- ggplot() + 
  ...
  geom_hline(yintercept = 0) + #geom_vline(xintercept = 0, linetype = 'dashed') +
  geom_line(data = pixel.data %>%
              filter(!is.na(Tree_Cover) & fire.year <= 2010 & stand.age >= 2 & !is.na(stand.age.bin)) %>% 
              group_by(date, stand.age.bin) %>%
              summarize(Tree_Cover.mean = mean(Tree_Cover), Tree_Cover.n = n()) %>%  
              filter(if_else(stand.age.bin == '1985-2010', Tree_Cover.n >= 11000 , Tree_Cover.n >= 0)),
...
f2
#Save the data
ggsave(filename = 'Fig3_dieoff_tree_cover_stand_age_time_series_frap_perimeter_50pt_sample.png', height=12, width= 14, units = 'cm', dpi=900)

#Create a Precip time series figure
p5 <- ggplot() + 
  geom_hline(yintercept = 0) +
  geom_line(data = pixel.data %>%
              filter(!is.na(ppt) & fire.year <= 2010 & stand.age >= 2 & !is.na(stand.age.bin)) %>% 
              ...
              filter(if_else(stand.age.bin == '1985-2010', count >= 11000, count >= 0)), 
...
p6 <- ggplot() + 
  geom_hline(yintercept = 0) +
  geom_line(data = pixel.data %>%
              filter(!is.na(AET) & fire.year <= 2010 & stand.age >= 2 & !is.na(stand.age.bin)) %>% 
                       # fire.year %notin% c(1985, 1986, 1987, 1988, 1989, 1990, 1991, 1992, 1993, 1994, 1995, 1996, 1997, 1998, 1999, 2000, 2001, 2002, 2003)) %>%
              group_by(date, stand.age.bin) %>%
...
p7 <- ggplot() + 
  ...
  geom_hline(yintercept = 0) + #geom_vline(xintercept = 0, linetype = 'dashed') +
  geom_line(data = pixel.data %>%
              filter(!is.na(Soil_Moisture) & fire.year <= 2010 & stand.age >= 2 & !is.na(stand.age.bin)) %>% 
              group_by(date, stand.age.bin) %>%
...
p8 <- ggplot() + 
  ...
  geom_hline(yintercept = 0) + #geom_vline(xintercept = 0, linetype = 'dashed') +
  geom_line(data = pixel.data %>%
              filter(!is.na(Water_Stress) & fire.year <= 2010 & stand.age >= 2 & !is.na(stand.age.bin)) %>% 
              group_by(date, stand.age.bin) %>%
...
f3
#Save the data
ggsave(filename = 'Fig4_water_stress_stand_age_frap_perimeter_50pt_sample_time_series.png', height=22, width= 16, units = 'cm', dpi=900)
...
ggsave(filename = 'Fig5_data_check_time_series_4group_0pt5pct.png', height=16, width= 16, units = 'cm', dpi=900)

#Creating a fire year dTree plot
p11 <- ggplot(data = pixel.data %>% dplyr::filter(fire.year <= 2010 & fire.year >= 1910) %>% dplyr::group_by(system.index) %>% summarize(dTree = Tree_Cover[vi.year == 2016] - Tree_Cover[vi.year == 2012], Water_Stress.4yr = Water_Stress[vi.year == 2015], stand.age.bin = stand.age.bin)) +
  ...
  theme_bw()
p11
ggsave(filename = 'Fig6_water_stress_stand_age_frap_50pt_sample.png', height=16, width= 18, units = 'cm', dpi=900)

# pixel.data %>% summary()
p12 <- ggplot(data = pixel.data %>% dplyr::filter(fire.year <= 2010 & fire.year >= 1910) %>% dplyr::group_by(system.index) %>% summarize(dTree = Tree_Cover[vi.year == 2016] - Tree_Cover[vi.year == 2012], Water_Stress = Water_Stress[vi.year == 2015], SPI48 = SPI48[vi.year == 2015], stand.age.bin = stand.age.bin)) +
...
ggsave(filename = 'Fig7_SPI48_stand_age_frap_50pt_sample.png', height=16, width= 18, units = 'cm', dpi=900)
...
ggsave(filename = 'Fig8_PrET4yr_stand_age_frap_50pt_sample.png', height=16, width= 18, units = 'cm', dpi=900)
...
#Rx Fire example
# p25 <- ggplot(data = pixel.data %>% dplyr::filter(!is.na(stand.age) & stand.age <= 25 & stand.age >= 0 & fire_type_2010 == 2) %>% 
#                 dplyr::group_by(system.index) %>% 
