...
fire.data <- read.csv(file.path(dir_in, "disturb_south_sierra_CCDC_FRAP_dist_100pts_100elev_200precip_5temp_ts8_300m_20221025.csv"), header = TRUE, na.strings = "NaN")
# fire.data$fire.year <- fire.data$perimeter_year
fire.data$treatment <- 'Disturb'
# list.files(fire_in)
# list.files(fire_in)
control.data <- read.csv(file.path(dir_in, "control_south_sierra_CCDC_FRAP_undist_100pts_100elev_200precip_5temp_ts8_300m_20221025.csv"), header = TRUE, na.strings = "NaN")
...
#   geom_hline(yintercept = 0) + #geom_vline(xintercept = 0, linetype = 'dashed') +
#   geom_point(data = pixel.data %>%
#                dplyr::filter((!is.na(Tree_Cover)) #&
#                              # if_else(treatment == 'Wildfire', fire.year == fire_year_2019_mode, is.na(fire_year_2019_mode))
#                ) %>%
#                group_by(date, fire.year, stratlayer) %>%
#                summarize(Tree_Cover.diff = mean(Tree_Cover[treatment == 'Disturb']) - mean(Tree_Cover[treatment == 'Control']), Tree_Cover.n = n(), fire.year = fire.year[treatment == 'Disturb']) %>%
#                dplyr::filter(fire.year <= 2010 & fire.year >= 1921),
#              # filter(if_else(fire.year.bin == '1981-2010', Tree_Cover.n >= 600, Tree_Cover.n >= 0)) %>%
#              # group_by(date, fire.year.bin) %>%
#              # summarize(Tree_Cover.diff = Tree_Cover.mean[treatment == 'Buffer'] - Tree_Cover.mean[treatment == 'Wildfire']), #%>%
#              # group_by(date, fire.year.bin) %>%
...
# 
# #Save the data
# ggsave(filename = 'Fig79_fire year_tree_cover_frap_perimeter.png', height=18, width= 20, units = 'cm', dpi=900)
...
p3 <- ggplot() + 
  geom_hline(yintercept = 0) +
  geom_line(data = pixel.data %>%
              filter((!is.na(tpa_max) & treatment == 'Disturb' & fire.year <= 2010 & fire.year >= 1921 & stand.age > 2) | (!is.na(tpa_max) & treatment == 'Control' & is.na(fire.year))) %>% # & 
              # elevation <= elev.upper & clm_precip_sum_mean >= ppt.lower & 
              # if_else(treatment == 'Wildfire', fire.year == fire_year_2019_mode, is.na(fire_year_2019_mode))) %>% 
              group_by(date, fire.year.bin) %>%
...
p4 <- ggplot() + 
  ...
  geom_line(data = pixel.data %>%
              ...
              group_by(date, fire.year.bin) %>%
              summarize(Tree_Cover.mean = mean(Tree_Cover), Tree_Cover.n = n()) %>%  
              filter(if_else(fire.year.bin == '1981-2010', Tree_Cover.n >= 1500, Tree_Cover.n >= 0)),
              mapping = aes(x = date, y = Tree_Cover.mean), 
              size = 1) + 
  #Tree Cover 95% CI
  geom_ribbon(data = pixel.data %>%
...
p5 <- ggplot() + 
  ...
  geom_line(data = pixel.data %>%
              ...
              filter(if_else(fire.year.bin == '1981-2010', count >= 1500, count >= 0)),
