...
ggsave(filename = 'Fig13_north_sierra_mortality_time_series_FIA.png', height=18, width= 10, units = 'cm', dpi=900)
...
p4 <- ggplot() + geom_point(data = join %>% filter(INVYR %in% c(2013,2014,2015,2016,2017,2018,2019) & STDAGE <= 250 & STDAGE > 0), mapping = aes(x = STDAGE, y = basal_area.all)) +
      theme_bw()
p4

p5 <- ggplot() + geom_point(data = join %>% filter(INVYR %in% c(2013,2014,2015,2016,2017,2018,2019) & STDAGE <= 250 & STDAGE > 0), mapping = aes(x = STDAGE, y = tpa.all)) +
      theme_bw()
p5

p6 <- ggplot() + geom_point(data = join %>% filter(INVYR %in% c(2013,2014,2015,2016,2017,2018,2019) & STDAGE <= 250 & STDAGE > 0), mapping = aes(x = STDAGE, y = basal_area.dead)) +
...
p7 <- ggplot() + geom_point(data = join %>% filter(INVYR %in% c(2013,2014,2015,2016,2017,2018,2019) & STDAGE <= 250 & STDAGE > 0), mapping = aes(x = basal_area.all, y = basal_area.dead)) +
  theme_bw()
p7

f2 <- ggarrange(p4, p5, p6, p7, ncol = 2, nrow = 2, common.legend = FALSE, align = "v", labels = c('a)', 'b)', 'c)', 'd)'))
f2

ggsave(filename = 'Fig14_mortality_standage_FIA.png', height=18, width= 18, units = 'cm', dpi=900)
