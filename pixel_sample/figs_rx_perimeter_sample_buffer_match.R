...
# list.files(fire_in)
# list.files(fire_in)
rx.control.data <- read.csv(file.path(dir_in, "fraprx_ecoregion_simple_sample_by_2km_rxfire_buffers_10pt_300m_ts8_20220927.csv"), header = TRUE, na.strings = "NaN")
...
# rx.control.data$fire.year <- rx.control.data$perimeter_year
rx.control.data$treatment <- '2-km Buffer' #Try making this 1-km versus, 2-km
...
rx.combine.data <- rbind(rx.fire.data, rx.control.data)
...
#Elevation limits
elev.lower <- (rx.pixel.data %>% filter(fire.year >= 1971 & fire.year <= 1980))$elevation %>% quantile(0.10)
elev.lower
elev.upper <- (rx.pixel.data %>% filter(fire.year >= 1971 & fire.year <= 1980))$elevation %>% quantile(0.80)
elev.upper
ppt.upper <- (rx.pixel.data %>% filter(fire.year >= 1971 & fire.year <= 1980))$clm_precip_sum_mean %>% quantile(0.80)
ppt.upper
ppt.lower <- (rx.pixel.data %>% filter(fire.year >= 1971 & fire.year <= 1980))$clm_precip_sum_mean %>% quantile(0.10)
ppt.lower
temp.lower <- (rx.pixel.data %>% filter(fire.year >= 1971 & fire.year <= 1980))$clm_temp_mean_mean %>% quantile(0.10)
temp.lower
...
p2 <- ggplot() + 
  ...
  #Create a shrub cover line
  geom_line(data = rx.pixel.data %>%
              filter(stand.age >= -10 & stand.age <= 35 & !is.na(Shrub_Cover) & vi.year <= 2014 & fire.year >= 1971 & fire.year <= 2010 & elevation <= elev.upper & elevation >= elev.lower & clm_precip_sum_mean <= ppt.upper & clm_precip_sum_mean >= ppt.lower & clm_temp_mean_mean >= temp.lower) %>%
              group_by(stand.age, treatment) %>%
  ...
  geom_line(data = rx.pixel.data %>%
              filter(stand.age >= -10 & stand.age <= 35 & !is.na(Tree_Cover) & vi.year <= 2014 & fire.year >= 1971 & fire.year <= 2010 & elevation <= elev.upper & elevation >= elev.lower & clm_precip_sum_mean <= ppt.upper & clm_precip_sum_mean >= ppt.lower & clm_temp_mean_mean >= temp.lower) %>%
              group_by(stand.age, treatment) %>%
              summarize(Tree_Cover.mean = mean(Tree_Cover)), mapping = aes(x = stand.age, y = Tree_Cover.mean, color = 'Tree'), size = 1) + 
  #Tree Cover 95% CI
  geom_ribbon(data = rx.pixel.data %>% 
                filter(stand.age >= -10 & stand.age <= 35 & !is.na(Tree_Cover) & vi.year <= 2014 & fire.year <= 2010 & fire.year >= 1971 & elevation <= elev.upper & elevation >= elev.lower & clm_precip_sum_mean <= ppt.upper & clm_precip_sum_mean >= ppt.lower & clm_temp_mean_mean >= temp.lower) %>%
                group_by(stand.age, treatment) %>%
  ...
  geom_line(data = rx.pixel.data %>%
              filter(stand.age >= -10 & stand.age <= 35 & !is.na(Herb_Cover) & vi.year <= 2014 & fire.year <= 2010 & fire.year >= 1971 & elevation <= elev.upper & elevation >= elev.lower & clm_precip_sum_mean <= ppt.upper & clm_precip_sum_mean >= ppt.lower & clm_temp_mean_mean >= temp.lower) %>%
              group_by(stand.age, treatment) %>%
  ...
  geom_line(data = rx.pixel.data %>%
              filter(stand.age >= -10 & stand.age <= 35 & !is.na(Bare_Cover) & vi.year <= 2014 & fire.year <= 2010 & fire.year >= 1971 & elevation <= elev.upper & elevation >= elev.lower & clm_precip_sum_mean <= ppt.upper & clm_precip_sum_mean >= ppt.lower & clm_temp_mean_mean >= temp.lower) %>% 
              group_by(stand.age, treatment) %>%
...
p2a <- ggplot() + 
  ...
  #Tree Cover 95% CI
  geom_ribbon(data = rx.pixel.data %>% 
                filter(stand.age >= -10 & stand.age <= 35 & !is.na(Tree_Cover) & vi.year <= 2014 & fire.year <= 2010 & fire.year >= 1971 & !is.na(fire.year) & elevation <= elev.upper & elevation >= elev.lower & clm_precip_sum_mean <= ppt.upper & clm_precip_sum_mean >= ppt.lower & clm_temp_mean_mean >= temp.lower) %>%
                group_by(stand.age, treatment) %>%
...
p9 <- ggplot() + 
  geom_hline(yintercept = 0) +
  geom_line(data = rx.pixel.data %>%
              filter(!is.na(Tree_Cover) & fire.year <= 2010 & fire.year >= 1971 & stand.age > 2 & elevation <= elev.upper & elevation >= elev.lower & clm_precip_sum_mean <= ppt.upper & clm_precip_sum_mean >= ppt.lower & clm_temp_mean_mean >= temp.lower) %>% 
...
p3 <- ggplot() + 
  geom_hline(yintercept = 0) +
  geom_line(data = rx.pixel.data %>%
              filter((!is.na(tpa_max) & fire.year <= 2010 & fire.year >= 1971 & stand.age > 2 & elevation <= elev.upper & elevation >= elev.lower & clm_precip_sum_mean <= ppt.upper & clm_precip_sum_mean >= ppt.lower & clm_temp_mean_mean >= temp.lower)) %>% # & vi.year >= 2003) %>%
              group_by(date, fire.year.bin, treatment) %>%
  ...
  #Dead Trees 95% CI
  geom_ribbon(data = rx.pixel.data %>%
                filter((!is.na(tpa_max) & fire.year <= 2010 & fire.year >= 1971 & stand.age > 2 & elevation <= elev.upper & elevation >= elev.lower & clm_precip_sum_mean <= ppt.upper & clm_precip_sum_mean >= ppt.lower & clm_temp_mean_mean >= temp.lower)) %>% 
                group_by(date, fire.year.bin, treatment) %>%
...
p4 <- ggplot() + 
  ...
  geom_line(data = rx.pixel.data %>%
              ...
              filter(if_else(fire.year.bin == '2001-2010', Tree_Cover.n >= 115, Tree_Cover.n >= 30)),
...
p5 <- ggplot() + 
  geom_hline(yintercept = 0) +
  geom_line(data = rx.pixel.data %>%
              filter((!is.na(ppt) & fire.year <= 2010 & fire.year >= 1971 & stand.age > 2 & elevation <= elev.upper & elevation >= elev.lower & clm_precip_sum_mean <= ppt.upper & clm_precip_sum_mean >= ppt.lower & clm_temp_mean_mean >= temp.lower)) %>% 
              ...
              filter(if_else(fire.year.bin == '2001-2010', count >= 115, count >= 30)), 
...
p6 <- ggplot() + 
  geom_hline(yintercept = 0) +
  geom_line(data = rx.pixel.data %>%
              filter((!is.na(AET) & fire.year <= 2010 & fire.year >= 1971 & stand.age > 2 & elevation <= elev.upper & elevation >= elev.lower & clm_precip_sum_mean <= ppt.upper & clm_precip_sum_mean >= ppt.lower & clm_temp_mean_mean >= temp.lower)) %>% 
                       # fire.year %notin% c(1985, 1986, 1987, 1988, 1989, 1990, 1991, 1992, 1993, 1994, 1995, 1996, 1997, 1998, 1999, 1800, 2001, 2002, 2003)) %>%
              group_by(date, fire.year.bin, treatment) %>%
  ...
  geom_rect(data = data.frame(xmin = as.Date('2011-10-01'), xmax = as.Date('2015-09-30'), ymin = -Inf, ymax = Inf),
            fill = "red", alpha = 0.3, mapping = aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax)) +
  xlim(as.Date('1985-08-01'),as.Date('2020-01-01')) + ylim(400, 800) + facet_grid(. ~ fire.year.bin) +
...
p7 <- ggplot() + 
  ...
  geom_hline(yintercept = 0) + #geom_vline(xintercept = 0, linetype = 'dashed') +
  geom_line(data = rx.pixel.data %>%
              filter((!is.na(Soil_Moisture) & fire.year <= 2010 & fire.year >= 1971 & stand.age > 2 & elevation <= elev.upper & elevation >= elev.lower & clm_precip_sum_mean <= ppt.upper & clm_precip_sum_mean >= ppt.lower & clm_temp_mean_mean >= temp.lower)) %>% 
              group_by(date, fire.year.bin, treatment) %>%
...
p8 <- ggplot() + 
  ...
  geom_hline(yintercept = 0) + #geom_vline(xintercept = 0, linetype = 'dashed') +
  geom_line(data = rx.pixel.data %>%
              filter((!is.na(Water_Stress) & fire.year <= 2010 & fire.year >= 1971 & stand.age > 2 & elevation <= elev.upper & elevation >= elev.lower & clm_precip_sum_mean <= ppt.upper & clm_precip_sum_mean >= ppt.lower & clm_temp_mean_mean >= temp.lower)) %>% 
              group_by(date, fire.year.bin, treatment) %>%
...
f3
#Save the data
ggsave(filename = 'Fig68_water_stress_stand_age_frap_perimeter_10pt_sample_300m_time_series.png', height=22, width= 16, units = 'cm', dpi=900)

#Summary of Prescribed fire impacts on die-off
# rx.pixel.data %>% filter(FIRE_NAME != "") %>% group_by(FIRE_NAME, treatment)
#Tree Cover Die-off
p14 <- ggplot() +
       #Data Summary
       geom_point(data = rx.pixel.data %>% dplyr::filter((fire.year <= 2010 & fire.year >= 1971 & stand.age > 2 & elevation <= elev.upper & elevation >= elev.lower & clm_precip_sum_mean <= ppt.upper & clm_precip_sum_mean >= ppt.lower & clm_temp_mean_mean >= temp.lower)) %>% 
       ...
       geom_errorbar(data = rx.pixel.data %>% dplyr::filter((fire.year <= 2010 & fire.year >= 1971 & stand.age > 2) & elevation <= elev.upper & elevation >= elev.lower & clm_precip_sum_mean <= ppt.upper & clm_precip_sum_mean >= ppt.lower & clm_temp_mean_mean >= temp.lower) %>% 
                       dplyr::group_by(system.index, treatment) %>% 
               summarize(dTree = (mean(Tree_Cover[vi.year %in% c(2018, 2019)]) - mean(Tree_Cover[vi.year %in% c(2014,2015)])), RdTree = (mean(Tree_Cover[vi.year %in% c(2018, 2019)]) - mean(Tree_Cover[vi.year %in% c(2014,2015)])) / mean(Tree_Cover[vi.year %in% c(2014, 2015)]), 
                         fire.year.bin = fire.year.bin[vi.year == 2010], Water_Stress = Water_Stress[vi.year == 2015]),
...
#Rx Fire Sample characteristics
p20 <- ggplot() +
  #Data Summary
  ...
  geom_histogram(data = rx.pixel.data %>% dplyr::filter((fire.year <= 2010 & fire.year >= 1971 & stand.age > 2)) %>%
                   dplyr::group_by(system.index, treatment) %>%
...
ggsave(filename = 'Fig70_rxfire_sample_characteristics_300m.png', height=28, width = 18, units = 'cm', dpi=900)
