...
--Sierra Nevada EcoRegions
-- (P.ECOSUBCD = 'M261Ep' OR P.ECOSUBCD = 'M261Eq' OR P.ECOSUBCD = 'M261Eu' OR P.ECOSUBCD = 'M261Er' OR P.ECOSUBCD = 'M261Eo' OR P.ECOSUBCD = 'M261Es') --South Sierra Nevada P.ECOSUBCD = 'M262Bb' OR P.ECOSUBCD = 'M262Ba' OR 
...
f1<- ggplot() + #geom_line(data = join %>% group_by(INVYR) %>% summarize(BA.all = mean(basal_area.all)), mapping = aes(x = INVYR, y = BA.all), color = 'green') + 
  ...
  geom_point(data = join %>% 
               filter(!is.na(disturb.bin) & disturb.bin != 'Die-off' & INVYR %in% c("2013","2014","2015", "2016", "2017", "2018", "2019") ) %>%  
...
join %>% 
  filter(!is.na(disturb.bin) & INVYR %in% c("2013","2014","2015", "2016", "2017", "2018", "2019") ) %>% 
...
f4<- ggplot() + #geom_line(data = join %>% group_by(INVYR) %>% summarize(BA.all = mean(basal_area.all)), mapping = aes(x = INVYR, y = BA.all), color = 'green') + 
  #Mean Die-off
  geom_point(data = join %>% 
               filter(!is.na(disturb.bin) & disturb.bin != 'Die-off' & (!is.na(STDAGE) & STDAGE != 9999) & INVYR %in% c("2013","2014","2015", "2016", "2017", "2018", "2019") ) %>% 
              group_by(disturb.bin) %>% 
