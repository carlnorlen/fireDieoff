...
p2 <- ggplot(data = filter(pixel.data, stand.age >= 0 & !is.na(dNDMI) & fire.year <= 2010 & fire_sev_last != 255), mapping = aes(x = date, y = dNDMI)) + 
  ...
  facet_grid(sev.bin ~ year.bin) + ylab('dNDMI') + xlab('Year') + 
...
p3 <- ggplot(data = filter(pixel.data, stand.age >= 0 & !is.na(Water_Stress) & fire.year <= 2010 & fire_sev_last != 255), mapping = aes(x = date, y = Water_Stress)) + 
  ...
  scale_fill_gradient2(limits = c(0,500), breaks = c(125,250,375), midpoint = 250, 
                       low = "cornflowerblue", mid = "yellow", high = "red", na.value = 'transparent') +
  facet_grid(sev.bin ~ year.bin) + ylab(expression('Water Stress (mm yr'^-1*')')) + xlab('Year') + 
...
p4 <- ggplot(data = filter(pixel.data, stand.age >= 0 & !is.na(Soil_Moisture) & fire.year <= 2010 & fire_sev_last != 255), mapping = aes(x = date, y = Soil_Moisture)) + 
  ...
  scale_fill_gradient2(limits = c(0,140), breaks = c(35,70,105), midpoint = 70, 
                       low = "cornflowerblue", mid = "yellow", high = "red", na.value = 'transparent') +
  facet_grid(sev.bin ~ year.bin) + ylab('Soil Moisture (mm)') + xlab('Year') + 
  theme_bw()
p4
...
ggsave(filename = 'Fig5_Biomass_fire_year_time_series.png', height=12.5, width= 20, units = 'cm', dpi=900)